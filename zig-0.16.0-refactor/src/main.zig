...
// ANSI color codes
const COLOR_COMMENT: u8 = 36; // cyan
const COLOR_MLCOMMENT: u8 = 36; // cyan
const COLOR_KEYWORD1: u8 = 33; // yellow
const COLOR_KEYWORD2: u8 = 32; // green
const COLOR_STRING: u8 = 35; // magenta
const COLOR_NUMBER: u8 = 31; // red
const COLOR_MATCH: u8 = 34; // blue
const COLOR_DEFAULT: u8 = 37; // white
...
fn posixWrite(fd: posix.fd_t, buf: []const u8) !usize {
    while (true) {
...
fn isKeyPrintable(key: KeyAction) bool {
    const k = @intFromEnum(key);
    return k < 128 and std.ascii.isPrint(@as(u8, @truncate(k)));
}

fn keyToChar(key: KeyAction) u8 {
    return @truncate(@intFromEnum(key));
}
