...
fn editorRowInsertChar(row: *EditorRow, at: usize, ch: u8) void {
    ...
    if (at > size) {
        ...
        var p: usize = 0;
        while (p < padlen) : (p += 1) {
            row.chars.append(gpa, ' ') catch return;
        }
...
fn editorRefreshScreen() void {
    ...
    if (msglen > 0 and getTimestamp() - E.statusmsg_time < 5) {
        const show_len = if (msglen <= E.screencols) msglen else E.screencols;
        ab.appendSlice(gpa, E.statusmsg[0..show_len]) catch return;
    }
